...
GUIExpInfo::GUIExpInfo( entt::registry& registry ) : registry( registry )
{
    backgroundBar.setFillColor( sf::Color::Black );
    backgroundBar.setSize(
        sf::Vector2f( 220 * ConfigManager::getInstance().getScale(), 20 * ConfigManager::getInstance().getScale() ) );
    backgroundBar.setOutlineThickness( 4 );
    backgroundBar.setOutlineColor( sf::Color::White );
    backgroundBar.setPosition(
        sf::Vector2f{ 180 * ConfigManager::getInstance().getScale(), 50 * ConfigManager::getInstance().getScale() } );
    ...
    skillPointsAvailable.setPosition(
        sf::Vector2f{ 160 * ConfigManager::getInstance().getScale(), 50 * ConfigManager::getInstance().getScale() } );
}
...
void GUIExpInfo::draw( Window& window )
{
    if ( this->skillPointsAvailableVisible )
    {
        window.draw( this->skillPointsAvailable );
    }
    window.draw( this->backgroundBar );
