...
void GUIDialogBox::updateText()
{
    ...
    {
        this->messageTexts[ i ].setOrigin( this->messageTexts[ i ].getGlobalBounds().getCenter() );
        this->messageTexts[ i ].setPosition( sf::Vector2f{
            this->box.getPosition().x, this->box.getPosition().y - this->box.getSize().y / 2 +
                                           40 * ConfigManager::getInstance().getScale() +
                                           i * 40 * ConfigManager::getInstance().getScale() } );
    }

    this->noText.setOrigin( this->noText.getGlobalBounds().getCenter() );
    this->noText.setPosition( sf::Vector2f{ this->box.getPosition().x - this->box.getSize().x / 4,
                                            this->box.getPosition().y + this->box.getSize().y / 2 -
                                                60 * ConfigManager::getInstance().getScale() } );

    this->yesText.setOrigin( this->yesText.getGlobalBounds().getCenter() );
    this->yesText.setPosition( sf::Vector2f{ this->box.getPosition().x + this->box.getSize().x / 4,
                                             this->box.getPosition().y + this->box.getSize().y / 2 -
                                                 60 * ConfigManager::getInstance().getScale() } );

    this->okText.setOrigin( this->okText.getGlobalBounds().getCenter() );
    this->okText.setPosition( sf::Vector2f{ this->box.getPosition().x,
                                            this->box.getPosition().y + this->box.getSize().y / 2 -
                                                60 * ConfigManager::getInstance().getScale() } );
}
...
void GUIDialogBox::setMessage( std::vector< std::string > message )
{
    ...
    auto findLongestLine = []( std::vector< sf::Text > texts ) -> sf::Text
    {
        ...
        for ( auto text : texts )
        {
            if ( text.getGlobalBounds().size.x > longestText.getGlobalBounds().size.x )
                longestText = text;
        }
